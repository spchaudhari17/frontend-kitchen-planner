...
const OrderProductDetails = () => {
  ...
  try {
    ...
    const response = await axios.get(`${process.env.REACT_APP_SERVER_URL}/api/payment/order-products/${transactionId}`);
    setProducts(response.data.data || []);
    setUserInfo(response.data.user || null);
...
if (response.data.user?._id) {
  const cartRes = await axios.get(`${process.env.REACT_APP_SERVER_URL}/api/cart/user/${response.data.user._id}`);
            ...
            <Card.Body style={{ minHeight: "150px" }}>
              ...
              <Card.Text>
                ...
                {Array.isArray(cartData) && cartData[index] && (
                  <>
                    <strong>Description:</strong> {cartData[index].description || "-"}<br />
                    <strong>Subdescription:</strong> {cartData[index].subdescription || "-"}<br />
                    <strong>Notes:</strong>
                    <ul>
                      {cartData[index].notes
                        ? Object.entries(cartData[index].notes).map(([key, val]) => (
                            <li key={key}>
